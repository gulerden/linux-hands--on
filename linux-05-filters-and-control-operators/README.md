...
- Create a `text` file named `new.txt`.
​
```bash
cat << EOF > new.txt
new:Road to reinvent yourself.
EOF
```

- Display the content of new.txt.
​
```bash
cat new.txt
```

- In the content of new.txt, replace or translate aer letters with 'QAZ'.
​
```bash
cat new.txt | tr aer QAZ
```
​
...
```
​
- Delete all the vowels in the content of new.txt.
​
```bash
cat new.txt | tr -d aeiou
```
​
- Write the whole content of new.txt in capital letters.
​
```bash
...
​
- Display days.txt or write 'new' on the screen, then write 'one'.
​
```bash
cat days.txt || echo new ; echo one
```
...
- EXERCISE:
​
  1.a. Search for “new.doc” in the current directory
    b. If it exists display its content
    c. If it does not exist print message “Too early!”
  2.Create a file named “new.doc” that contains “Congratulations”
  3.Repeat Step 1
​
```bash
********************************
