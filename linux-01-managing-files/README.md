...
- Create a `text` file named `faruk.txt`.
...
- Show the first 10 lines of faruk.txt.

```bash
head faruk.txt
```

- Show the first 5 lines of faruk.txt.

```bash
head -5 faruk.txt
```

- Show the last 10 lines of faruk.txt.

```bash
tail faruk.txt
```

- Show the last 5 lines of faruk.txt.

```bash
tail -5 faruk.txt
```

- Display the faruk.txt file on the screen.
...
- View the faruk.txt file with the `more` command.

```bash
more faruk.txt
```

- View the faruk.txt file with the `less` command.
...
- Display faruk.txt file in reverse.

```bash
tac faruk.txt
```

- Create reverse-faruk.txt in reverse of faruk.txt.

```bash
tac faruk.txt > reverse-faruk.txt
```
...
- Find all the files whose name is faruk.txt in a current working directory.

```bash
find . -name faruk.txt
```

- Find all the files under /home directory with name faruk.txt.

```bash
find /home -name faruk.txt
```

- Find all the files whose name is faruk.txt and contains both capital and small letters in /home directory.

```bash
find /home -iname faruk.txt
```
...
- Sometimes we need more content in search results to decide what is most relevant. For this, we can use the following operators to add the desired lines before, after a match, or both:
    ...
    ```bash
    grep -A3 "line 5" faruk.txt # this command prints three lines after the match.
    ```

    - Use -B and a number of lines to display before a match.
    
    ```bash
    grep -B2 "line 5" faruk.txt # this command prints two lines before the match.
    ```

    - Use -C and a number of lines to display before and after the match.
    
    ```bash
    grep -C4 "line 5" faruk.txt # this command prints four lines before and after the match.
    ```
