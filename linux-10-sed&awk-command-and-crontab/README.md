# Hands-on Linux-10 : sed & new command and crontab

Purpose of the this hands-on training is to teach the students how to use sed & new command and crontab.

## Learning Outcomes

At the end of the this hands-on training, students will be able to;

- use sed & new command and crontab.
...
- Create a folder and name it `sed-new-command`.

```bash
mkdir sed-new-command && cd sed-new-command
```
...
- new is a text pattern scanning and processing language, created by Aho, Weinberger & Kernighan (hence
...
- While the sed program works well with character-based processing, the new program works well with delimited field processing.

- Create a file named `new.txt`. 
...
> new options 'selection _criteria {action }' file

- By default new prints every line of data from the specified file.
...
By default, the new command splits the record delimited by a whitespace character.  new assigns some variables for each data field as below:
...
- We can change delimiter by using –F option. First, update the new.txt as below.
...
- We can use new command as filter. 
